...
set.seed(123)
rf <- randomForest(fdom ~ . -date, data = train_data, ntree = 300, mtry=6)
...
library(kknn)
set.seed(123)
knn_model <- kknn::train.kknn(fdom ~ . -date, data = train_data, ks = 3) #
...
if (case_study=="sau"){
  ...
  w1 <- 0.423 #linear
  w2 <- 0.577 #lgb
  blended<- w1 * pred_lm + w2 * pred_lgb
  blended_training <- w1 * preds_training$LM + w2 * preds_training$LGB
}
