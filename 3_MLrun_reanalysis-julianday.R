...
write.csv(results_df, file=paste0(dir,"output/metrics_models_testing_onlyreanalysis.csv"),
          quote = F, row.names = F)
...
write.csv(results_df, file=paste0(dir,"output/metrics_models_training_onlyreanalysis.csv"),
          quote = F, row.names = F)
...
points(df_test_pred$Date[df_test_pred$Model==model_name_toplot], 
       df_test_pred$Value[df_test_pred$Model==model_name_toplot], col="limegreen", pch = 19, cex=0.5)
