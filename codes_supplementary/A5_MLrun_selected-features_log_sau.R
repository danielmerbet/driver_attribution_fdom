...
if (case_study=="sau"){
  ...
  data$fdom <- log10(data$fdom) 
}

#load hyperparameters
load(paste0(dir, "output/hyperparameters/log/tuned_models.rdata"))
...
write.csv(importance(rf)/sum(importance(rf)), 
          file=paste0(dir,"output/importance/log/imp_rf.csv"),
          quote = F)
...
write.csv(xgb.importance(model = xgb_model), 
          file=paste0(dir,"output/importance/log/imp_xgb.csv"),
          quote = F)

# Save XGBoost model
xgb.save(xgb_model, paste0(dir, "output/models/log/xgb_model.model"))
...
write.csv(lgb.importance(lgb_model), 
          file=paste0(dir,"output/importance/log/imp_lgb.csv"),
          quote = F)

# Save LightGBM model to file
lgb.save(lgb_model, paste0(dir, "output/models/log/lgb_model.txt"))
...
write.csv(data.frame(var=colnames(X_train),
                     importance=catboost.get_feature_importance(cat_model)/sum(catboost.get_feature_importance(cat_model))), 
          file=paste0(dir,"output/importance/log/imp_cbt.csv"),
          quote = F)

# Save CatBoost model in binary format
catboost.save_model(cat_model,
                    model_path = paste0(dir, "output/models/log/cat_model.cbm"),
                    file_format = "cbm")
...
write.csv(results_df, file=paste0(dir,"output/log/metrics_models_testing.csv"),
          quote = F, row.names = F)
...
write.csv(results_df, file=paste0(dir,"output/log/metrics_models_training.csv"),
          quote = F, row.names = F)
...
title = ""
filename = paste0(dir, "output/log/model_comparison_train_test_selected.pdf")
...
pdf(paste0(dir, "output/log/simulation1_",model_name_toplot,".pdf"), width = 8, height = 6)
plot(obs_df_plot$Date, 10^obs_df_plot$Value, ylab="fDOM (QSU)", 
...
points(df_train_pred$Date[df_train_pred$Model==model_name_toplot], 
       10^df_train_pred$Value[df_train_pred$Model==model_name_toplot], col="steelblue", pch = 19, cex=0.5)
points(df_test_pred$Date[df_test_pred$Model==model_name_toplot], 
       10^df_test_pred$Value[df_test_pred$Model==model_name_toplot], col="magenta4", pch = 19, cex=0.5)

if(model_name_toplot=="Blended"){
  points(df_train_pred$Date[df_train_pred$Model=="Blended"], 
         10^df_train_pred$Value[df_train_pred$Model=="Blended"], col="steelblue", pch = 19, cex=0.5)
  points(df_test_pred$Date[df_test_pred$Model=="Blended"], 
         10^df_test_pred$Value[df_test_pred$Model=="Blended"], col="magenta4", pch = 19, cex=0.5)
}

text(x = min(df_train_pred$Date[df_train_pred$Model==model_name_toplot]), 
     y = max(10^df_train_pred$Value[df_train_pred$Model==model_name_toplot]-10),
