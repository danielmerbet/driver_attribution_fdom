...
set.seed(123)
rf <- randomForest(fdom ~ . -date, data = train_data, ntree = 1000, mtry=tuned_models$rf$bestTune$mtry)
...
library(kernlab)
set.seed(123)
svr_model <- ksvm(fdom ~ . -date, data = train_data, kernel = "rbfdot", C = tuned_models$svr$bestTune$C, sigma=tuned_models$svr$bestTune$sigma)
