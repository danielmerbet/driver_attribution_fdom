...
case_study <- "sau"
model_name_toplot <- "Blended"#c("RF", "XGB", "LGB", "Catboost", "Linear", "KNN", "SVR", "Blended)

dir <- paste0(here(),"/",case_study, "/")
...
if (case_study=="sau"){
  ...
  eps <- 1e-6  # safety for zeros
  data$fdom_log <- log10(data$fdom + eps)
  inv_log10 <- function(x) 10^x
}
...
# Remove date
X_train <- train_data %>% select(-fdom, -fdom_log, -date)
y_train <- train_data$fdom_log

X_test <- test_data %>% select(-fdom, -fdom_log, -date)
y_test <- test_data$fdom_log
...
set.seed(123)
rf <- randomForest(fdom_log ~ . -fdom -date, data = train_data, ntree = 1000, 
                   mtry=tuned_models$rf$bestTune$mtry)
pred_rf_log <- predict(rf, newdata = test_data)
pred_rf     <- inv_log10(pred_rf_log)
preds_training <- cbind(preds_training,
                        inv_log10(predict(rf, newdata = train_data)))
...
pred_xgb <- inv_log10(pred_xgb_log)
xgb.importance(model = xgb_model)
preds_training <- cbind(preds_training,
                        inv_log10(predict(xgb_model, newdata = dtrain)))
...
pred_lgb_log <- predict(lgb_model, newdata = as.matrix(X_test))
pred_lgb <- inv_log10(pred_lgb_log)
preds_training <- cbind(preds_training, 
                        inv_log10(predict(lgb_model, newdata = as.matrix(X_train))))
...
cat_model <- catboost.train(learn_pool = train_pool, 
                            params = params)
pred_ctb_log <- catboost.predict(cat_model, pool = test_pool)
pred_ctb <- inv_log10(pred_ctb_log)
preds_training <- cbind(preds_training,
                        inv_log10(catboost.predict(cat_model, pool = train_pool)))
...
#liner regresion
set.seed(123)
lm_model <- lm(fdom_log ~ . -fdom -date, data = train_data)
pred_lm_log <- predict(lm_model, newdata = test_data)
pred_lm <- inv_log10(pred_lm_log)
preds_training <- cbind(preds_training, 
                        inv_log10(predict(lm_model, newdata = train_data)))
#doesn't directly provide feature importance scores

library(kknn)
set.seed(123)
knn_model <- kknn::train.kknn(fdom_log ~ . -fdom -date, data = train_data, ks = tuned_models$knn$bestTune$k) #
pred_knn_log <- predict(knn_model, newdata = test_data)
pred_knn <- inv_log10(pred_knn_log)
preds_training <- cbind(preds_training, 
                        inv_log10(predict(knn_model, newdata = train_data)))
#doesn't directly provide feature importance scores

library(kernlab)
set.seed(123)
svr_model <- ksvm(fdom_log ~ . -fdom -date, data = train_data, kernel = "rbfdot", 
                  C = tuned_models$svr$bestTune$C, sigma=tuned_models$svr$bestTune$sigma)
pred_svr_log <- predict(svr_model, newdata = test_data)
pred_svr <- inv_log10(pred_svr_log)
preds_training <- cbind(preds_training, 
                        inv_log10(predict(svr_model, newdata = train_data)))
#doesn't directly provide feature importance scores
...
nn_model <- nnet(
  fdom_log ~ . -fdom -date,
  data = train_data,
  size = 5,        # hidden neurons (tuneable)
  decay = 1e-3,    # L2 regularization
  maxit = 2000,
  linout = TRUE,   # regression
  trace = FALSE
)

pred_nn_log <- predict(nn_model, newdata = test_data)
pred_nn <- inv_log10(pred_nn_log)

preds_training <- cbind(
  preds_training,
  inv_log10(predict(nn_model, newdata = train_data))
)
...
y_train_actual <- train_data$fdom
y_test_actual  <- test_data$fdom
results <- list()

evaluate <- function(name, pred, obs) {
...
evaluate("RF", pred_rf, obs = y_test_actual)
evaluate("XGB", pred_xgb, obs = y_test_actual)
evaluate("LGB", pred_lgb, obs = y_test_actual)
evaluate("CTB", pred_ctb, obs = y_test_actual)
evaluate("LM", pred_lm, obs = y_test_actual)
evaluate("KNN", pred_knn, obs = y_test_actual)
evaluate("SVR", pred_svr, obs = y_test_actual)
evaluate("NN", pred_nn, obs = y_test_actual)
...
evaluate("Blended", blended, obs=y_test_actual)
...
evaluate("RF", preds_training$RF, obs=y_train_actual)
evaluate("XGB", preds_training$XGB, obs=y_train_actual)
evaluate("LGB", preds_training$LGB, obs=y_train_actual)
evaluate("CTB", preds_training$CTB, obs=y_train_actual)
evaluate("LM", preds_training$LM, obs=y_train_actual)
evaluate("KNN", preds_training$KNN, obs=y_train_actual)
evaluate("SVR", preds_training$SVR, obs=y_train_actual)
...
evaluate("Blended", blended_training, obs=y_train_actual)
...
for (model_name in names(train_predictions_list)) {
  train_metrics_list[[model_name]] <- list(
    R2 = r2(y_train_actual, train_predictions_list[[model_name]]),
    RMSE = rmse(y_train_actual, train_predictions_list[[model_name]]),
    NSE = nse(y_train_actual, train_predictions_list[[model_name]]),
    KGE = kge(y_train_actual, train_predictions_list[[model_name]])
  )
  
  test_metrics_list[[model_name]] <- list(
    R2 = r2(y_test_actual, test_predictions_list[[model_name]]),
    RMSE = rmse(y_test_actual, test_predictions_list[[model_name]]),
    NSE = nse(y_test_actual, test_predictions_list[[model_name]]),
    KGE = kge(y_test_actual, test_predictions_list[[model_name]])
  )
}
